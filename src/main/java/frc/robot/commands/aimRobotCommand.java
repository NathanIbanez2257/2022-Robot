...
public class aimRobotCommand extends CommandBase 
{
  ...
  errorDistance, oldErrorDistance, speedDistance,               // error values, distance forward || backwards, speed Distance = PID finalization
  ...
  */
  ...
  // called continuously as command is ran
  @Override
  public void execute() 
  {

    // run PID with no D, between error of [__] degrees +-

    if(drive.angleOff() < 20)
    {

      // error aim = targetAngle(0) + (angle displacement)
      ...
      errorDistance = td -  drive.currentDistance();                      // error distance = total distance - currentDistance 
